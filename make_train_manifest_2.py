...
for entry in test_data:
    ...
    if lpips > 0.1 or flolpips > 0.1:
        video = entry['video']
        # Extract frame identifier from gt path (e.g., 00001 from /path/.../00001.jpg)
        frame_id = os.path.splitext(os.path.basename(entry['gt']))[0]
        data_id = f"{video}_{frame_id}"

        # Adjust prediction path
        prediction_path = entry['prediction'].replace(
            '/home/cx24957/VFI/GenVFI/', '/mnt/e/genvfi_outputs/')
...
# Save to output JSON
output_path = '/mnt/e/Difix3d/data/converted_dataset_fixed.json'
